...
## ***Intro***
Loop만 봐도 Pythonic한 우아함을 실감할 수 있는데요. 여기에 list comprehension이라는 녀석을 더하면 그 매력은 두말할 나위가 없어집니다. Python에서 list comprehension으로 작업을 하다가 이 list comprehension을 지원 안하는 다른 언어에서 같은 일을 하려면 정말 짜증부터 나거든요.

먼저 list는 알겠는데 comprehension은 뭐냐 그러실 수도 있겠어요. 이렇게 갸웃하는 분들이 있을 것 같은데 사실 이 list comprehension이라는 표현은 영어 원어민 프로그래머들도 어원을 잘 몰라요. 

그냥 list comprehension 자체를 하나의 새로운 용어로 받아들이는 게 속편할 겁니다. 그래도 영어쟁이니까 굳이 이해하고 싶다면 원래 set comprehension이라고, 집합 이론에서 나온 표현이고요. 저 멀리 조상 언어들까지 거슬러 올라가야 되거든요. Lisp 같은 거. 동사 comprehend의 기본 그림은 이렇게 함께 잡아서 통째로 이해한다는 건데요. 우리 말로 '파악하다' 한자말 있죠.  파악하다. 파악이라는 한 자가 손으로 꽉 잡아서 내 것으로 만드는 그림이거든요. 그래서 영어에 grasp라는 단어가 '파악하다'와 그림이 똑같습니다. grasp가 꽉 잡는 거거든요. 그래서 파생적인 의미로 뭔가 완전히 이해를 할 때 'grasp 한다'고 하거든요. 둘 다 원래 손으로 꽉 쥐는 게 기본 그림이죠. 그래서 comprehend는 com, 같이 함께, together 이런 뜻이죠. prehend, 잡는다는 뜻입니다. 그래서 통째로 이해한다는 뜻이고요. 또 comprehend의 두 번째 그림은 '포함', include거든요. 그러니까 **주어진 list를 완전히 이해해서 조건을 만족하는 새로운 list를 만들어낸다는 그림**으로 이해해도 되고요. **주어진 조건을 만족하는 원소들을 몽땅 포함해서 새로운 list를 만든다**고 이해해도 됩니다. 그런데 아까도 말했듯이 영어권 원어민 컴쟁이들도 의견 통일이 안 되는 표현이니까 굳이 어원까지 파고들 필요는 없어요. 그냥 우리나라 컴쟁이들도 list comprehension이라고 합니다. 굳이 번역할 필요가 없거든요. 번역도 안되고요. 그런데 이걸 또 어거지로 집합 축약이니 list 축약이니 하는 표현을 쓰기도 하는데 난 도무지 용납이 안 돼요. 도무지 용납이 안 되는 표현이라 나는 반댈세입니다. 축약은 아니거든요. 그냥 list comprehension이라고 합시다. 이상한 번역하지 말고. 외래어에요 외래어. 외래어로 받아들이세요. list comprehension.
