...
## ***Generator***
오늘은 Generator 라는 녀석에 관해서 짚어보도록 하겠습니다. Generator를 간단히 요약하면 일반 함수랑 달리 한 번만 값을 `return`하는 게 아니라 연속적인 값을 return하는 iterator 역할을 한다는 겁니다. Generator를 정의하는 방법은 함수랑 똑같아요. 단지 `return` 문을 쓰지 않고 `yield` 문을 쓴다는 것만 다른데요. 
